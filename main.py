...
def game():
    ...
    #random choice of template:
  
    index = [0,1,2]
    temp = random.choice(index)
  
    #Conditions (index) for inputes:
  
    if temp == 0:
        
        number_1 = input(Fore.BLUE + "Number /1: ")
        ...
        #Conditions for a/an article:
      
        if adjective_1:
          if adjective_1[0].lower() in ('a', 'e', 'i', 'o', 'u'):
              a_an = "an"
  
          elif adjective_1[0].lower() in ('b', 'c', 'd', 'f', 'g', 'j', 'k', 'l', 'm', 'n', 'p', 'q', 's', 't', 'v', 'x', 'z', 'h', 'r', 'w', 'y'):
              a_an = "a"
        ...
        pn = input(Fore.BLUE + "Proper Noun (Person’s Name): ")
        noun_1 = input("Noun /1: ")
...
# start/fnish function
def play():
    
    answer = input(Back.GREEN + 'Hello. You have the chance to play a very interesting game. Do you want to start (Y/N)?: ' + Style.RESET_ALL)

    if answer.lower() == "y":
        print("\n\n" + Style.BRIGHT + "Mad Libs Game starts" + Style.RESET_ALL + "\n\nPlease enter a values for the following words:\n ")
        game()

    elif answer.lower() == "n":
        print('\n' + Fore.YELLOW + "Hope you will have other chance")

    else:
        con = input("\n" + Fore.RED + "Incorrect answer. If you wnat to continue click on" + Style.RESET_ALL + Fore.CYAN + "'ENTER'" + Style.RESET_ALL + "\n\n")
        if con == "":
            play()
        else:
          print(Fore.RED + "\n" + "Incorrect answer --- FINISH")
